...
if(Form->ShowModal()==mrOk)
 ...
 if(i==maxgraph)
  {
  Application->MessageBox("Максимальное количество осей","Сообщение",MB_OK|MB_ICONSTOP);
  delete Form;
  return;
  }
 for(int j=0;j<i;j++)
 if(PrintedAxis[j]==Form->Signal)
  {
  Application->MessageBox("Такая ось уже присутствует","Сообщение",MB_OK|MB_ICONSTOP);
  delete Form;
  return;
  }
