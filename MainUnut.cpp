...
do
 ...
 if(DataModule1->KanalsTable->FieldByName("OffSet")->AsString=="")
  ...
  {
  Application->MessageBox("Превышение допустимого номера сигнала","Warning",MB_OK|MB_ICONSTOP);
  }
  //*/
 ...
 if(MaxSoundNumber>0)
  ...
  (MainForm->MediaPlayer->Position==MainForm->MediaPlayer->Length)))
   ...
   try
    ...
    {
    Application->MessageBox("Со звуком что-то не то.","Warning",MB_OK|MB_ICONWARNING);
    }
   }
...
if(AdConnThread->CommErrorInfo.Error)
 {
 CommError+="Нет ответа с";
 for(int i=0;i<maxadam;i++)
  if(AdConnThread->CommErrorInfo.FailsPorts[i])
   {
   CommError+=" :Порт "+IntToStr(i)+" каналы";
   bool Flag=0;
   for(int j=0;j<maxchannel;j++)
...
AnsiString MesAn="Свяжитесь с поризводителем или через "+
...
if(Tim>=Message) Application->MessageBox(&Mes[0],"Сообщение",
...
do
{
 ...
 if(FileExists(SaveDialog->FileName))
  {
  AnsiString Mes="Файл "+SaveDialog->FileName+" уже существует. Заменить существующий  файл?";
  char* Mes1=Mes.c_str();
  if(MessageBox(NULL,Mes1,"Сохранить как",MB_OKCANCEL|MB_ICONQUESTION)==IDOK)
   {
...
float CurrentValue;
MainForm->StatusBar->SimpleText="Запись базы данных";
MainForm->StatusBar->SimplePanel=true;
try
{
//DataModule1->KanalsTable->DisableControls();
...
if(((Now>=RefreshTime1)&(Now<=RefreshTime2))&&(!OnlyOperate))
 ...
 StatusBar->SimpleText="Обновление базы данных и резервное копирование";
...
DataModule1->StatistTable->Active=true;
Application->MessageBox("Индексы созданы успешно",
"Сообщение",MB_OK|MB_ICONEXCLAMATION|MB_APPLMODAL);
}
catch(...)
{
DataModule1->StatistTable->Active=false;
DataModule1->StatistTable->Exclusive=false;
DataModule1->StatistTable->Active=true;
Application->MessageBox("Для создания новых индексов необходимо сначала удалить старые",
"Сообщение",MB_OK|MB_ICONERROR|MB_APPLMODAL);
}
//*/
}
//---------------------------------------------------------------------------
